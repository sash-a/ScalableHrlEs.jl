...
function run_hrl_nses(name::String, cnns, pnn, envs, comm::Union{Comm,ScalableES.ThreadComm}; obstat = nothing,
    ...
    onehot = false, prim_specific_obs = false, behv_freq = 25, min_w = 0.8)
    ...
    evalfn = (nns, e, obmean, obstd) -> first(first(first(hrl_ns_run_env(nns, e, obmean, obstd, interval, steps, 10, cdist;
        cforward = cforward, rng = nothing, behv_freq = behv_freq, prim_specific_obs = prim_specific_obs, 
        maze_eval = eval_maze, earlystop_eval = eval_gather))))
    ...
    ScalableES.run_gens(gens, name, ps, nt, f, behvfn, evalfn, envs, npolicies, opt, obstat, tblg, steps, episodes, w_schedule, behv_freq, comm)
...
function hrl_ns_run_env(nns::Tuple{Chain,Chain}, env, (cobmean, pobmean), (cobstd, pobstd),
    ...
    cforward = onehot_forward, prim_specific_obs = false, behv_freq = 25,
    ...
    for ep = 1:episodes
        ((ep_cr, ep_cpath), (ep_pr, ep_ppath)), ep_step, (ep_cobs, ep_pobs) =
            _ns_one_episode(nns, env, (cobmean, pobmean), (cobstd, pobstd),
                ...
                prim_specific_obs = prim_specific_obs, behv_freq = behv_freq,
...
function _ns_one_episode(nns::Tuple{Chain,Chain}, env, (cobmean, pobmean), (cobstd, pobstd),
    cintervals::Int, steps::Int, episodes::Int, cdist::Float32;
    behv_freq=25, cforward = onehot_forward, prim_specific_obs = false,
    rng = Random.GLOBAL_RNG, earlystop_eval = false, maze_eval = false)
    ...
    for i = 0:steps-1
        ...
        if behv_freq < 0 && i == steps - 1  # if behv_freq == -1 then only record final position
            push!(cpath, Vec2(pos...))
            push!(ppath, Vec2(pos...))
        end
