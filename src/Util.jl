...
ScalableES.sumsteps(res::Vector{HrlEsResult{T}}) where T = ScalableES.sumsteps(map(r->r.pres, res))  # only counting the steps of the primitive
...
function ScalableES.rank(results::Vector{HrlEsResult{T}}) where T
...
function StatsBase.summarystats(results::Vector{HrlEsResult{T}}) where T
    StatsBase.summarystats(map(r->r.cres, results)), StatsBase.summarystats(map(r->r.pres, results))
end

function angle_encode_target(targetvec::Vector{T}, torso_ang::T) where T <: AbstractFloat
